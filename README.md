...
- Замена одинакова везде — в GUI и в терминалах: демону не нужны выделение
  и буфер обмена, каретка гарантированно стоит сразу после набранного слова.
- Границы, после которых фикс безопасен: пробел и пунктуация; Enter/Tab,
  стрелки, Ctrl/Alt-комбо сбрасывают буфер без фикса (текст мог уже
  «уехать» — выполненная команда, автодополнение).
...
Известные ограничения v1: автоповтор зажатой клавиши не учитывается в
буфере (слова, набранные с долгим зажатием, не анализируются — фикс не
сработает, лишнего не сделает); быстрая печать прямо в момент инъекции
может перемешаться с ней (без grab перехватить нечего); паузная граница
может разрезать слово, набранное с длинной заминкой — поэтому пауза по
умолчанию 1000 мс, а слова короче 3 символов демон не трогает вовсе
(`daemon.min_word_len`).
...
boundary_pause_ms  = 1000 # пауза, после которой слово считается законченным
                          # (щедрая нарочно: короткая рвёт слова на обрывки)
