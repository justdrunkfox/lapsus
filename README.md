...
- **Готовое выделение уважается.** Если в момент хоткея уже есть
  выделение (мышь или Shift+стрелки) — чинится именно оно: ввод заменяет
  активное выделение, Ctrl+Shift+Left не нажимается. Нечего чинить —
  выделение остаётся нетронутым; выделено не одно слово — отказ без
  порчи текста. Нет выделения — чинится слово слева от каретки
  (как раньше). Проверено вживую в waterfox (Shift+Home + хоткей).
